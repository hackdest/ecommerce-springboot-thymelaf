...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Modifying
    @Transactional
    @Query("UPDATE Product p SET p.currentQuantity = :qty WHERE p.id = :id")
    void updateQty(@Param("qty") int qty, @Param("id") Long id);

    @Query("SELECT p.currentQuantity FROM Product p WHERE p.id = :id")
